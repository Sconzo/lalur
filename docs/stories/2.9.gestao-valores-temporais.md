...
- [ ] Criar Entidade ValorParametroTemporal (AC: 1)
  - [ ] Criar `ValorParametroTemporalEntity`
  - [ ] Adicionar validação `@PrePersist` e `@PreUpdate`
  - [ ] Adicionar UNIQUE constraint
  - [ ] Configurar tabela e colunas

- [ ] Criar Repository (AC: 2)
  - [ ] Criar `ValorParametroTemporalJpaRepository`
  - [ ] Adicionar métodos customizados de busca

- [ ] Criar DTOs
  - [ ] Criar `CreateTemporalValueRequest`
  - [ ] Criar `TemporalValueResponse`
  - [ ] Criar `TimelineResponse`

- [ ] Criar Use Cases (AC: 7)
  - [ ] Implementar `CreateTemporalValueUseCase`
  - [ ] Implementar `ListTemporalValuesUseCase`
  - [ ] Implementar `DeleteTemporalValueUseCase`
  - [ ] Implementar `GetCompanyTaxParametersTimelineUseCase`

- [ ] Criar endpoints (AC: 3, 4, 5, 6)
  - [ ] Criar `POST .../temporal-values`
  - [ ] Criar `GET .../temporal-values?ano=`
  - [ ] Criar `DELETE .../temporal-values/{id}`
  - [ ] Criar `GET .../tax-parameters-timeline?ano=`
  - [ ] Adicionar `@PreAuthorize("hasRole('ADMIN')")`

- [ ] Implementar formatação de período (AC: 4)
  - [ ] Criar método para formatar "Jan/2024", "1º Tri/2024"

- [ ] Implementar query agregada (AC: 6)
  - [ ] Query com JOINs triplos
  - [ ] Agrupamento por tipo de parâmetro
  - [ ] Ordenação por mes/trimestre

- [ ] Atualizar CompanyResponse (AC: 8)
  - [ ] Adicionar campo `hasTemporalValues`

- [ ] Configurar cascade delete (AC: 9)
  - [ ] Adicionar ON DELETE CASCADE no DDL

- [ ] Criar testes de integração (AC: 10)
  - [ ] Testar criação mensal e trimestral
  - [ ] Testar validação de periodicidade
  - [ ] Testar unique constraint
  - [ ] Testar listagem e filtros
  - [ ] Testar timeline agregada
  - [ ] Testar cascade delete
  - [ ] Testar segurança (CONTADOR 403)
